 ...
 * 		>Jpa+Repository 라는 어노테이션이 따로 있음
 * 	- 제공되는 기본 기능 : save(), findById(), findAll(), delete() 등
 * 	- 추상 메서드를 입력해야 서비스 클래스에서 사용가능 (request.getUserId()) 등
 */
public interface MemberRepository extends JpaRepository<Member, Long> {

//	boolean 사용하는 이유 : 참과 거짓으로 검증하기 때문
	/**
	 * 아이디 중복 체크
	 * 	- JPA 메서드 이름 규칙에 다라 자동으로 쿼리가 생성됨
	 ...
	 * @return true : 이미 해당 사용자가 email 를 사용중
	 * 		   false : 존재하지 않음 (사용 가능)
	 */
	 ...
	 * 
	 * 참고
	 * 	- 여러 개의 SELECT 결과를 받을 경우 List 가 사용됨
